-- ============================================================
-- InvestTrack - Case- and whitespace-insensitive investor name matching
--
-- Both cross-project linking RPCs matched investors by exact name
-- equality. That caused duplicates in the cross-project running
-- totals when the same person was entered with different casing or
-- stray whitespace ("Venkatesh Nanneboina" vs "venkatesh nanneboina"
-- vs " Venkatesh  Nanneboina ").
--
-- Fix: compare with lower(trim(name)) so the link survives those
-- variations. Add a partial index to keep the lookups fast.
-- ============================================================

-- Speed up the new matching for projects with many investors
create index if not exists investors_normalized_name_idx
  on investors (project_id, (lower(trim(name))));

-- 1. process_loan_repayment — match destination investor case-insensitively
  ...
  if v_total_contributed > 0 then
    for v_contribution in (
      ...
      if p_type = 'project_adjustment' and p_to_project_id is not null then
        -- CHANGED: case- and whitespace-insensitive match
        select id into v_dest_investor_id
        from investors
        where project_id = p_to_project_id
          and lower(trim(name)) = lower(trim(v_contribution.investor_name))
        limit 1;
...
-- 2. reallocate_investor_position — same fix
...
begin
  ...
  where project_id = p_dest_project_id
    and lower(trim(name)) = lower(trim(v_source_name))
  limit 1;

  if v_dest_investor_id is null then
    raise exception 'No investor named "%" on the destination project (matched case-insensitively). Add them there first.', v_source_name;
  end if;
...
-- 3. Optional data hygiene: trim leading/trailing whitespace on existing
-- investor names so the index works for existing rows.
update investors
set name = trim(name)
